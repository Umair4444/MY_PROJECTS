...
async function game() {
  let user = await inquirer
    ...
    .then((user) => {
      let random_number: number = Math.ceil(Math.random() * 6);

      if (user.my_input === random_number) {
        console.log(
          chalk.green(`You Guess Right Number that is ${user.my_input}`)
        );
      } else {
        console.log(
          chalk.red(
            `Your Answer is Wrong your guess is ${user.my_input} and computer was thinking ${random_number}`
          )
        );
      }
    });
